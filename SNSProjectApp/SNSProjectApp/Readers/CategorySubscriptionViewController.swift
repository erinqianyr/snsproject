...
//        Alamofire.request(MyVariables.url + "/get_categories", method: .get, encoding: URLEncoding.default){
//            
//        }
    }
    ...
    @objc func subscribe(sender: UIButton) {
        ...
        print(JSONString!)
        let p: [String: Any] = [
            "subscriptions": JSONString!,
            "rid": Login_vars.id
        ]
        
        Alamofire.request(MyVariables.url + "/subscribe", method: .post, parameters:p, encoding: URLEncoding.default)
            .responseData{ response in
                print(response)
        }
